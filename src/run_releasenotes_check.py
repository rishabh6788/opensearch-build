...
def main() -> int:
    ...
    if args.action == "check":
        ...
        #print(f"file path is {os.path.abspath(os.path.dirname(__file__))}")
        #sys.exit(0)
        for manifest in manifests:
            product = manifest.build.name
            for component in manifest.components.select(focus=components, platform='linux'):
                release_notes.generate(product, component, manifest.build.version, manifest.build.qualifier)
